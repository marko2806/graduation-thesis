...
def convert_yolov5_to_coco(yolov5_output):
    ...
    coco_output['boxes'] = torch.Tensor(np.array([x.xyxy.cpu().numpy() for x in yolov5_output.boxes]))
    coco_output['boxes'] = coco_output['boxes'].reshape((-1, 4))
    coco_output['scores'] = torch.Tensor(np.array([x.conf.cpu().numpy() for x in yolov5_output.boxes]).reshape(-1))
    coco_output['labels'] = torch.IntTensor(np.array([np.ones_like(x.conf.cpu().numpy()) for x in yolov5_output.boxes]).reshape(-1))
    return coco_output
...
if __name__ == "__main__":
    ...
    sku110kDataset = SKU110kDataset(
        "./datasets/SKU110K", get_transform(train=False), "val")
